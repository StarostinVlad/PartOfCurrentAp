...
class DefectsFingerprint(private val onSelect: (DefectData) -> Unit) :
    ...
    override fun getViewHolder(
        ...
        return ItemDefectViewHolder(binding, onSelect)
    }
}

class ItemDefectViewHolder(binding: ItemDefectBinding, val onSelect: (DefectData) -> Unit) :
    ...
    override fun onBind(item: DefectData) = with(binding) {
        ...
        when (item.status) {
            true -> btnChangeDefectStatus.text = "Удалить"
